...
class TimeKeepingView extends GetView<TimeKeepingController> {
  const TimeKeepingView({Key? key}) : super(key: key);
  @override
    ...
    return Expanded(
      ...
      child: DefaultTextStyle(
          style: const TextStyle(
              color: Colors.black, fontFamily: 'IBMPlexMono', fontSize: 18),
          child: Obx(
            () => ListView.builder(
              itemBuilder: (context, index) {
                // 技术点：有序数列，首位插入效率低
                // 将索引做下翻转
                int reverseIndex = (controller.records.length - 1) - index;
                bool isLightIndex =
                ...
                return Row(
                  children: [
                    ...
                    Text(controller.durationToString(
...
class Painter extends CustomPainter {
  ...
  void drawText(Canvas canvas) {
    ...
    textPainter.text = TextSpan(text: commonStr, style: textStyle, children: [
      TextSpan(
          text: highlightStr, style: textStyle.copyWith(color: themeColor)),
    ]);
