...
channel：
Flutter 与原生之间通信主要通过 Platform Channels，包含三种方式：

MethodChannel：最常用，适用于 Flutter 主动调用原生功能，比如调用摄像头、获取电量。通信是异步的，Flutter 发起请求，原生执行后返回结果。

EventChannel：适用于原生持续发送数据给 Flutter，例如蓝牙扫描回调、传感器数据变化。

BasicMessageChannel：用于双向消息传递，适合持续性或自定义消息通信，一般用于 Flutter 与原生共享状态。
...
如：Element 调用 unmount 时会同时触发 state 的 dispose() 方法
